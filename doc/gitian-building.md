...
*Setup instructions for a gitian build of suda using a VM or physical system.*

Gitian is the deterministic build process that is used to build the suda
...
to the suda GitHub release page.
...
- [Building suda](#building-suda-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/suda-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/suda-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/suda-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/suda-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/suda-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/suda-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/suda-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@suda-tv.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/suda-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/suda-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/suda-osx-*-build.assert
```
