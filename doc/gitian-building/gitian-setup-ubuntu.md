...
The build script we'll be using is contained in the suda github repository ([contrib/gitian-build.py](https://github.com/SUDATV/suda/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the suda repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/SUDATV/suda/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, suda-detached-sigs, and suda GitHub repos.
