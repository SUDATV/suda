...
bool CSUDAStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CSUDAStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CSUDAStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CSUDAStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CSUDAStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CSUDAStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CSUDAStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CSUDAStake::GetUniqueness() const
{
    //The unique identifier for a SUDA stake is the outpoint
...
CBlockIndex* CSUDAStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CSUDAStake::ContextCheck(int nHeight, uint32_t nTime)
{
