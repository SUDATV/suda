...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SUDA);
    ...
    if (settings.contains("nAnonymizesudaAmount") || reset)
        SoftSetArg("-anonymizesudaamount", settings.value("nAnonymizesudaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in suda.cpp during shotdown to also remove the window positions
