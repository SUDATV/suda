...
#include "qt/suda/forms/ui_send.h"
#include "qt/suda/addnewcontactdialog.h"
#include "qt/suda/qtutils.h"
#include "qt/suda/sendchangeaddressdialog.h"
#include "qt/suda/optionbutton.h"
#include "qt/suda/sendconfirmdialog.h"
#include "qt/suda/myaddressrow.h"
#include "qt/suda/guitransactionsutils.h"
...
SendWidget::SendWidget(sudaGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-suda");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination sudaAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(sudaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(sudaAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
