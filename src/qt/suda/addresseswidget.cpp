...
#include "qt/suda/forms/ui_addresseswidget.h"
#include "qt/suda/addresslabelrow.h"
#include "qt/suda/addnewaddressdialog.h"
#include "qt/suda/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(sudaGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination sudaAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(sudaAdd, label.toUtf8().constData(),
