...
#include "qt/suda/dashboardwidget.h"
#include "qt/suda/forms/ui_dashboardwidget.h"
#include "qt/suda/sendconfirmdialog.h"
#include "qt/suda/txrow.h"
...
DashboardWidget::DashboardWidget(sudaGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSUDA, "square-chart-suda");
    setCssProperty(ui->labelSUDA, "text-chart-suda");
    ...
    setCssProperty(ui->labelAmountSUDA, "text-stake-suda-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &sudaGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &sudaGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &sudaGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            suda = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalSUDA += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSUDA.append(suda);

        int max = std::max(suda, suda);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesSUDA);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSUDA > 0) {
        setCssProperty(ui->labelAmountSUDA, "text-stake-suda");
    } else {
        setCssProperty(ui->labelAmountSUDA, "text-stake-suda-disable");
    }
    forceUpdateStyle({ui->labelAmountSUDA});
    ui->labelAmountSUDA->setText(GUIUtil::formatBalance(chartData->totalSUDA, nDisplayUnit));
