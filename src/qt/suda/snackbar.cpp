...
#include "qt/suda/forms/ui_snackbar.h"
#include "qt/suda/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(sudaGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &sudaGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
