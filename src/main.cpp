...
#if defined(NDEBUG)
#error "suda cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usuda) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateSUDASupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating SUDA supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating SUDA supply..."), percent);
		}
...
{
    util::ThreadRename("suda-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> sudaInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                sudaInputs.push_back(stakeIn);
        }
        const bool hasSUDAInputs = !sudaInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasSUDAInputs) {
                    ...
                    for (const CTxIn& sudaIn : sudaInputs)
                        if(sudaIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasSUDAInputs) {
                            for (const CTxIn& stakeIn : sudaInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // suda: We use certain sporks during IBD, so check to see if they are
